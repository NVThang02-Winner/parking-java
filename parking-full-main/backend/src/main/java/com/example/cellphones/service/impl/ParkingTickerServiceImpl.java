...
public class ParkingTickerServiceImpl implements ParkingTickerService {
    ...
    public Object create(CreateTicketRequest req) {
        try {
            ...
            double price;
            if(req.getTicketType() == TicketType.MONTH){
                price = vehicleCategory.getPriceMonth() * req.getNumberTimeRent();
            }else{
                price = vehicleCategory.getPriceYear() * req.getNumberTimeRent();
            }
